...
# Fully aligned with your newest apps JSON format
GAME_MAP = {
    "567slot": {"api": "567slots", "ui": "567 Slots"},
    "MBMBet": {"api": "mbmbet", "ui": "MBM Bet"},
    "Bingo": {"api": "bingo101", "ui": "Bingo101"},
    "789Jackpot": {"api": "789jackpots", "ui": "789 Jackpots"},
    "SpinCrush": {"api": "spincrush", "ui": "Spin Crush"},
    "HiRummy": {"api": "hirummy", "ui": "Hi Rummy"},
    "MahaGame": {"api": "mahagames", "ui": "Maha Games"},
    "YonoVip": {"api": "yonovip", "ui": "Yono Vip"},
    "789Jackpots": {"api": "789jackpots", "ui": "789 Jackpots"},
    "MaxRummy": {"api": "maxrummy", "ui": "Max Rummy"},
    "YonoGames": {"api": "yonogames", "ui": "Yono Games"},
    "INDRummy": {"api": "indrummy", "ui": "IND rummy"},
    "YonoSlots": {"api": "yonoslots", "ui": "Yono Slots"}
}
...
live_stats = {
    ...
    "cancel_failed_numbers_list": []  # Holds precise numbers failing 4sim cancel verification
}
...
async def terminate_4sim_order_async(txn_id, otp_received, phone, force_cancel=False):
    ...
    if final_status in ["Released", "Finished Successfully", "Already Cancelled"]:
        ...
        # Requirement 2: Log exact number in Cancel Failed Logs card only when all 6 validation retries collapse
        async with stats_lock:
...
async def handle_already_number(phone, txn_id, otp_flag):
    ...
    # Requirement 1: Maps directly under Realtime Already Card analytics dashboard
    async with stats_lock:
...
async def run_game_step_async(phone, txn_id, game_key, used_otps, is_sub_game=False):
    ...
    while send_attempt <= 6:
        try:
            ...
            send_url = f"{BASE_URL}/yono?app={api_name}&action=sendotp&number={phone}&accesscode={SPYEYE_API_KEY}"
            response_raw = await loop.run_in_executor(None, lambda: requests.get(
                send_url, verify=False, timeout=12
            ))
            ...
            if v_res.get("success") is not True and v_res.get("status") != "success":
                ...
                display_err = v_res.get("msg") or v_res.get("message") or f"Err Code {response_raw.status_code}"
                await update_live_status(phone, f"API Hold: {display_err[:22]}", progress_val=5)
                
                if "rate limit" in error_msg or response_raw.status_code == 429:
            ...
            if v_res.get("success") is True or v_res.get("status") == "success":
                ...
                for attempt in range(1, max_attempts + 1):
                    ...
                    if otp:
                        ...
                        verify_url = f"{BASE_URL}/yono?app={api_name}&action=verify&requestid={request_id}&otp={otp}&accesscode={SPYEYE_API_KEY}"
                        verify_res = await loop.run_in_executor(None, lambda: requests.get(
                            verify_url, verify=False, timeout=15
                        ).json())
                ...
                if not otp_found_flag and not is_sub_game:
                    await update_live_status(phone, "Canceling SPYEYE...", progress_val=10)
                    try:
                        cancel_url = f"{BASE_URL}/yono?app={api_name}&action=cancel&requestid={request_id}&accesscode={SPYEYE_API_KEY}"
                        cancel_res = await loop.run_in_executor(None, lambda: requests.get(
                            cancel_url, verify=False, timeout=10
                        ).json())
            ...
            await update_live_status(phone, f"Err: Connect Dropped", progress_val=5)
            send_attempt += 1
            await asyncio.sleep(4)
            
    if last_known_request_id and not is_sub_game:
        await update_live_status(phone, "Max Fail: Canceling...", progress_val=5)
        try:
            cancel_url = f"{BASE_URL}/yono?app={api_name}&action=cancel&requestid={last_known_request_id}&accesscode={SPYEYE_API_KEY}"
            await loop.run_in_executor(None, lambda: requests.get(cancel_url, verify=False, timeout=10))
        except: pass
...
async def process_single_registration():
    ...
    if main_res == "already":
        ...
        other_games = ["MBMBet", "YonoGames", "YonoVip", "SpinCrush", "789Jackpots", "YonoSlots", "HiRummy", "Bingo", "MahaGame"]
