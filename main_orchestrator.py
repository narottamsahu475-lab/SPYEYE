...
FOUR_SIM_API_KEY = "525083da39ec7c93e1ebbfe1f6c5ec7d"
SPYEYE_API_KEY = "TESTSUMTHA00"
# Base URL with /up_yono/ path prefix as per API docs
BASE_URL = "https://spyeyeloots.online/up_yono"

# Google Sheets Config (Optional)
SHEET_NAME = "SPYEYE_Registrations"
sheet = None
...
async def run_game_step_async(phone, txn_id, game_key, used_otps, active_requests_tracker, success_chains_count, registered_games_list, game_balances_map, is_sub_game=False):
    if game_key not in GAME_MAP:
        return "failed", False, "0 INR"

    max_attempts = 9 if is_sub_game else 22
    send_attempt = 1
        ...
        while send_attempt <= 5:
            try:
                ...
                if v_res.get("success") is True or v_res.get("status") == "success":
                    ...
                    active_requests_tracker[game_key] = {"request_id": request_id, "api_name": api_name, "ui_name": ui_name}
                    ...
                    for attempt in range(1, max_attempts + 1):
                        ...
                        if otp:
                            ...
                            if verify_res.get("success") is True or verify_res.get("status") == "success":
                                ...
                                asyncio.create_task(append_to_google_sheet(phone, api_name, bal, dev_id, get_ist_time(), pass_word, uid_val))
                                
                                if game_key in active_requests_tracker:
                                    del active_requests_tracker[game_key]
                                ...
                                used_otps.add(otp)
                                if is_sub_game:
                                    display_v_msg = verify_res.get("msg") or "Wrong OTP"
                                    await update_live_status(phone, "Wrong OTP, Routing Back...", progress_val=85)
                                    
                                    routed_successfully = False
                                    for old_game_key, track_data in list(active_requests_tracker.items()):
                                        if old_game_key == game_key or old_game_key == "567slots":
                                            continue
                                        try:
                                            fallback_res = await spyeye_client.verify_otp(session, track_data["api_name"], track_data["request_id"], otp)
                                            if fallback_res.get("success") is True or fallback_res.get("status") == "success":
                                                f_bal_val = fallback_res.get("balance") or fallback_res.get("data", {}).get("account_balance", 0)
                                                f_bal = "₹" + str(int(float(f_bal_val)))
                                                
                                                f_dev_id = fallback_res.get("deviceid") or fallback_res.get("device_id") or ""
                                                f_pass = fallback_res.get("password") or ""
                                                f_uid = fallback_res.get("uid") or ""
                                                
                                                success_chains_count[0] += 1
                                                registered_games_list.append(old_game_key)
                                                game_balances_map[track_data["ui_name"]] = f_bal
                                                
                                                await log_game_metric(track_data["ui_name"], "success")
                                                await add_timeline_event(phone, f"Recovered Late OTP -> {track_data['ui_name']}")
                                                
                                                asyncio.create_task(append_to_google_sheet(phone, track_data["api_name"], f_bal, f_dev_id, get_ist_time(), f_pass, f_uid))
                                                
                                                if old_game_key in active_requests_tracker:
                                                    del active_requests_tracker[old_game_key]
                                    ...
                                    if routed_successfully:
                                        await update_live_status(phone, "Late OTP Recovered! Waiting New...", progress_val=40)
                                        continue
                                    else:
                    ...
                    if not otp_found_flag:
                        ...
                        await add_error_log(phone, ui_name, "OTP Timeout window reached")
                        await log_game_metric(ui_name, "failed")
                        
                    return "timeout", otp_found_flag, "0 INR"

            except Exception as e:
                await update_live_status(phone, "Err: Connection Exception", progress_val=5)
                await add_error_log(phone, ui_name, f"{type(e).__name__}: {str(e)[:100]}")
                send_attempt += 1
...
async def process_single_registration():
    ...
    main_res, m_otp_flag, main_bal = await run_game_step_async(
        phone, txn_id, "567slots", used_otps, active_requests_tracker, 
    ...
    other_games = ["YonoSlots", "SpinCrush", "YonoVip", "Bingo", "789Jackpots", "MBMBet", "HiRummy"]
    for game in other_games:
        sub_res, s_otp_flag, sub_bal = await run_game_step_async(
            phone, txn_id, game, used_otps, active_requests_tracker, 
            success_chains_count, registered_games_list, game_balances_map, is_sub_game=True
        )
        if s_otp_flag: 
            otp_received_anywhere = True
    ...
    async with aiohttp.ClientSession() as cleanup_session:
        for old_key, track_data in list(active_requests_tracker.items()):
            try:
                await spyeye_client.cancel_request(cleanup_session, track_data["api_name"], track_data["request_id"])
            except:
                pass
